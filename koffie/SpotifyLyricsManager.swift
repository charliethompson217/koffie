...
class SpotifyLyricsManager {
    ...
    func fetchLyricsForSong(song: String, artist: String, completion: @escaping ([LyricLine]?, Error?) -> Void) {
        fetchSpotifyToken { [weak self] accessToken in
            guard let self = self, let accessToken = accessToken else {
                completion(nil, NSError(domain: "Token Error", code: -1, userInfo: nil))
                return
            }
            self.fetchTrackId(song: song, artist: artist, accessToken: accessToken) { trackId in
                guard let trackId = trackId else {
                    completion(nil, NSError(domain: "Track ID Error", code: -1, userInfo: nil))
                    return
                }
                self.fetchLyrics(trackId: trackId, accessToken: accessToken) { lyricsData, error in
                    ...
                    if let lyrics = lyricsData["lyrics"] as? [String: Any], let lines = lyrics["lines"] as? [[String: Any]] {
                        ...
                        completion(lyricLines, nil)
                    } else {
                        completion(nil, NSError(domain: "Parsing Error", code: -1, userInfo: nil))
                    }
                }
            }
        }
    ...
    private func fetchTrackId(song: String, artist: String, accessToken: String, completion: @escaping (String?) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if let json = try? JSONDecoder().decode(SpotifySearchResponse.self, from: data),
               let trackId = json.tracks.items.first?.id {
                completion(trackId)
            } else {
